...
namespace QuanLyBanGiay_ADMIN.Controllers
{
	public class CheckoutController : Controller
	{
		...
		[HttpGet]
		public IActionResult AddBill(string ngaydathang, string phuongthucthanhtoan, string diachigh, string TenNguoidung, string SodienthoaiNv, double tongtienhoadon)
		{
			int mand = 0;
			...
			newbill.Thanhtien = (int?)tongtienhoadon;
			...
			foreach (var item in Cart.Lines)
			{
				...
				string masize = item.Product.size.ToString();
				SanPham_Size sanpham_size = _context.SanPham_Sizes.FirstOrDefault(sp => sp.MaSanpham == item.Product.Masanpham && sp.Ma_Size == masize);
				sanpham_size.SoLuong -= item.Quantity;
				_context.SanPham_Sizes.Update(sanpham_size);
				_context.SaveChanges();
			}
		...
		public IActionResult AddCommentToDB(string masanpham, string comment, int numstar, int makhachhang)
		{
			if (HttpContext.Session.GetString("Username") is not null)
			{
				...
				_context.SaveChanges();
				return Json(new { status = "Đánh giá của bạn đã được lưu lại" });
			}
			return RedirectToAction("Index", "PageNotFound");
		...
		[HttpGet]
		public JsonResult DeleteBill(string mahoadon)
		{
			var hoadonToUpdate = _context.Hoadons.FirstOrDefault(cthd => cthd.MaHoadon == mahoadon);

			if (hoadonToUpdate != null)
			{
				hoadonToUpdate.TrangthaiHD = 3;
				_context.SaveChanges();
			}
